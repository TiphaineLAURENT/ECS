...
{
        MySystem1() : System(ecs::SYSTEM_PRIORITY::LOWEST) {}
        void update(ecs::Interval deltaTime) override { std::cout << deltaTime << " for 1\n"; }
};

struct SYSTEM(MySystem2)
{
        MySystem2() : System(ecs::SYSTEM_PRIORITY::NORMAL, 5) {}
        void update(ecs::Interval deltaTime) override { std::cout << deltaTime << " for 2\n"; }
};

struct SYSTEM(MySystem3)
{
        MySystem3() : System(ecs::SYSTEM_PRIORITY::HIGHEST, 1) {}
        void update(ecs::Interval deltaTime) override { std::cout << deltaTime << " for 3\n"; }
};

TEST_CASE("Basic creation", "creation")
{
        auto entity = ecs::EntityManager::create<MyEntity>();
        REQUIRE(ecs::EntityManager::get_entity_container<MyEntity>().get_entity_type_name()
        ...
        REQUIRE(system1.get_update_interval() == 1.);
        ...
        REQUIRE(systemManager[0]->get_priority() == ecs::SYSTEM_PRIORITY::HIGHEST);
        REQUIRE(systemManager[1]->get_priority() == ecs::SYSTEM_PRIORITY::NORMAL);
        REQUIRE(systemManager[2]->get_priority() == ecs::SYSTEM_PRIORITY::LOWEST);
